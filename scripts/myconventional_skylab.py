...
def main(filename, image_dir, dtg=None, window_length=None, field=None):
    ...
    elif 'raob' in filename or 'sonde' in filename:
...
def go_raob(filename, image_dir, cdtg=None, window_length=6, field=None):
    ...
    if not cdtg:
        try:
            str_dtg = f.attrs['datetimeReference'].item()
            dtg = datetime.strptime(str_dtg, "%Y-%m-%dT%H:%M:%SZ")
        except:
            try:
                str_dtg = f.attrs['date_time_string'].decode("utf-8")   #V1
                dtg = datetime.strptime(str_dtg, "%Y-%m-%dT%H:%M:%SZ")
            except:
                try:
                    str_dtg = re.search("\d{8}T\d{2}Z", os.path.basename(filename)).group()
                    dtg = datetime.strptime(str_dtg, "%Y%m%dT%HZ")
                except:
                    try:
                        str_dtg = re.search("\d{10}", os.path.basename(filename)).group()
                        dtg = datetime.strptime(str_dtg, "%Y%m%d%H")
                    except:
    ...
    for plvl in [500]:
        ...
        try:
            air_pressure = np.array( f['MetaData']['pressure'] )
        except:
            try:
                air_pressure = np.array( f['MetaData']['air_pressure'] )
            except:
                print ('could not determine pressure vertical coordinate')
        ...
        for k in f['ObsValue'].keys():
            # surface pressure, station elevation and geopotential height do not have H(x)
            if ('surface' in k or 'Surface' in k or 'Station' in k or 'station' in k): continue
            print ( '      ....   plotting: ', k )
            ob = f['ObsValue'][k]
            try:
                hofx = f['hofx'][k]
                omb = np.array(f['ObsValue'][k]) - np.array(f['hofx'][k])
            except:
                hofx = ob
                omb = ob
            plots = { 'ob':  {
            ...
            for k_plot in plots:
                ...
                if k_plot != 'ob' and data == ob: continue
                data = data[ locs_01 ]
                data = data[ locs_02 ]
