...
if 'occulting_sat_id' in df.columns:
    ...
    for var in variables:
        ...
        for group in groups:
            ...
            xtitle = f"{group} platforms Bending Angle {win_beg.strftime('%Y%m%dT%HZ_PT6H')}"
            ...
            xtitle = f"{group} platforms H(x) {win_beg.strftime('%Y%m%dT%HZ_PT6H')}"
            plot_global(data = data, lat  = alat, lon = alon,
                        title = xtitle,
            ...
            xtitle = f"{group} platforms Observation - H(x) {win_beg.strftime('%Y%m%dT%HZ_PT6H')}"
            plot_global(data = data, lat  = alat, lon = alon,
                        title = xtitle,
