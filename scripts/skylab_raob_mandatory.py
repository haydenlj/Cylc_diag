...
#                       print ( "%s:  %f" % (ioda_var_name[k], xiv) )
                    ...
                    local['fg_depar'][ location_3d ] += xiv
                    local['stdv_fg_depar'][ location_3d ] += xiv**2
                    local['zonal_cnt'][ location_4d ]  += 1.0
                    local['zonal_fg_depar'][ location_4d ] += xiv
                    local['zonal_stdv_fg_depar'][ location_4d ] += xiv**2

                    # an_depar
                    local['an_depar'][ location_3d ] += an_depar
                    local['stdv_an_depar'][ location_3d ] += an_depar**2
                    local['zonal_an_depar'][ location_4d ] += an_depar
                    local['zonal_stdv_an_depar'][ location_4d ] += an_depar**2
...
#               sys.exit()
            ...
            count = np.ma.masked_less_equal( local['cnt'], 1 )
            local['fg_depar'] = np.ma.array( local['fg_depar'], mask = count.mask, fill_value = MISSING ) / count
            local['stdv_fg_depar'] = \
            np.sqrt(  np.ma.array( local['stdv_fg_depar'], mask = count.mask, fill_value = MISSING ) / count - \
                        local['fg_depar']**2 )
            local['an_depar'] = np.ma.array( local['an_depar'], mask = count.mask, fill_value = MISSING ) / count
            local['stdv_an_depar'] = \
                    np.sqrt(  np.ma.array( local['stdv_an_depar'], mask = count.mask, fill_value = MISSING ) / count - \
                        local['an_depar']**2 )
            count = np.ma.masked_less_equal( local['zonal_cnt'], 1 )
            local['zonal_fg_depar'] = np.ma.array( local['zonal_fg_depar'], mask = count.mask, fill_value = MISSING ) / count
            local['zonal_stdv_fg_depar'] = \
                    np.sqrt(  np.ma.array( local['zonal_stdv_fg_depar'], mask = count.mask, fill_value = MISSING ) / count - \
                        local['zonal_fg_depar']**2 )
            local['zonal_an_depar'] = np.ma.array( local['zonal_an_depar'], mask = count.mask, fill_value = MISSING ) / count
            local['zonal_stdv_an_depar'] = \
                    np.sqrt(  np.ma.array( local['zonal_stdv_an_depar'], mask = count.mask, fill_value = MISSING ) / count - \
                        local['zonal_an_depar']**2 )
            ...
            if cloop == 'fg_depar':
                ...
                d['cnt']            +=  local['cnt']
                d['fg_depar']       +=  local['cnt'] * local['fg_depar']
                d['zonal_cnt']      +=  local['zonal_cnt']
                d['zonal_fg_depar'] +=  local['zonal_cnt'] * local['zonal_fg_depar']
                ### an_depar
                d['an_depar']       +=  local['cnt'] * local['an_depar']
                d['zonal_an_depar'] +=  local['zonal_cnt'] * local['zonal_an_depar']
                ### ob sensitivity
                d['sens_cnt']       +=  local['sens_cnt']
                d['ob_sens']        +=  local['ob_sens']
                d['zonal_ob_sens']  +=  local['zonal_ob_sens']
                d['zonal_sens_cnt'] +=  local['zonal_sens_cnt']
            else:
                # weight stdv from date time group by its count
                count  = np.ma.masked_less_equal( local['cnt'], 2 )
                mult_zero = np.ma.array(count, mask=count.mask, fill_value=0.0)
                d['stdv_fg_depar']  +=  mult_zero.filled() * (local['stdv_fg_depar']**2 + (d['fg_depar']-local['fg_depar'])**2)
                d['stdv_an_depar']  +=  mult_zero.filled() * (local['stdv_an_depar']**2 + (d['an_depar']-local['an_depar'])**2)
                zonal_count  = np.ma.masked_less_equal( local['zonal_cnt'], 2 )
                mult_zonal_zero = np.ma.array(zonal_count, mask=zonal_count.mask, fill_value=0.0)
                d['zonal_stdv_fg_depar'] +=  mult_zonal_zero.filled() * (local['zonal_stdv_fg_depar']**2 + (d['zonal_fg_depar'] - local['zonal_fg_depar'])**2)
                d['zonal_stdv_an_depar'] +=  mult_zonal_zero.filled() * (local['zonal_stdv_an_depar']**2 + (d['zonal_an_depar'] - local['zonal_an_depar'])**2)
        ...
        if cloop == 'fg_depar':
            d['cnt'] = np.ma.masked_less_equal( d['cnt'], 1 )
            divide_one = np.ma.array(d['cnt'], mask=d['cnt'].mask, fill_value=1.0)
            # compute the global and zonal fg_depars
            d['fg_depar'] = np.ma.array( d['fg_depar'], mask = d['cnt'].mask, fill_value = MISSING ) / divide_one.filled()
            d['an_depar'] = np.ma.array( d['an_depar'], mask = d['cnt'].mask, fill_value = MISSING ) / divide_one.filled()
            d['zonal_cnt'] = np.ma.masked_less_equal( d['zonal_cnt'], 1 )
            zero_zonal_sum = np.ma.array(d['zonal_cnt'], mask=d['zonal_cnt'].mask, fill_value=0.0)
            divide_zonal_one = np.ma.array(d['zonal_cnt'], mask=d['zonal_cnt'].mask, fill_value=1.0)
            d['zonal_fg_depar'] = np.ma.array( d['zonal_fg_depar'], mask = d['zonal_cnt'].mask, fill_value = MISSING ) / divide_zonal_one.filled()
            d['zonal_an_depar'] = np.ma.array( d['zonal_an_depar'], mask = d['zonal_cnt'].mask, fill_value = MISSING ) / divide_zonal_one.filled()

            # and now observation sensitivity global and zonal fg_depars
            d['sens_cnt'] = np.ma.masked_less_equal( d['sens_cnt'], 1 )
            d['ob_sens'] = np.ma.array( d['ob_sens'], mask = d['sens_cnt'].mask, fill_value = MISSING)
            d['zonal_sens_cnt'] = np.ma.masked_less_equal( d['zonal_sens_cnt'], 1 )
            d['zonal_ob_sens'] = np.ma.array( d['zonal_ob_sens'], mask = d['zonal_sens_cnt'].mask, fill_value = MISSING)
        else:
            # compute the global and zonal stdv
            d['cnt'] = np.ma.masked_less_equal( d['cnt'], 2 )
            divide_one = np.ma.array(d['cnt'], mask=d['cnt'].mask, fill_value=1.0)
            d['stdv_fg_depar'] = np.sqrt( np.ma.array( d['stdv_fg_depar'], mask = d['cnt'].mask, fill_value=0. ).filled() / divide_one.filled() )
            d['stdv_an_depar'] = np.sqrt( np.ma.array( d['stdv_an_depar'], mask = d['cnt'].mask, fill_value=0. ).filled() / divide_one.filled() )
            d['zonal_cnt'] = np.ma.masked_less_equal( d['zonal_cnt'], 2 )
            divide_zonal_one = np.ma.array(d['zonal_cnt'], mask=d['zonal_cnt'].mask, fill_value=1.0)
            d['zonal_stdv_fg_depar'] = np.sqrt( np.ma.array( d['zonal_stdv_fg_depar'], mask=d['zonal_cnt'].mask, fill_value=0. ).filled() / 
                                             divide_zonal_one.filled() )
            d['zonal_stdv_an_depar'] = np.sqrt( np.ma.array( d['zonal_stdv_an_depar'], mask=d['zonal_cnt'].mask, fill_value=0. ).filled() / 
                                             divide_zonal_one.filled() )
    ...
    for var in sorted_var_keys:
        ...
        for include in ['normal']:
            ...
            for l, k in enumerate(sorted_raob_keys):
                ...
                if not d['sens_cnt'][ :, l, str_index ].sum():
                    junk = 1  # np.sum will reset tot_sens to mask if all values are masked
                else:
                    tot_sens += np.sum( d['sens_cnt'][ :, l, str_index ] )
                if np.max( np.absolute( d['ob_sens'][ :, l, str_index ] ) ) > rnd:
                    rnd = np.max( np.absolute( d['ob_sens'][ :, l, str_index ] ) ) 
            ...
            ########################################################
            ###      plot global mean and stdv of departures     ###
            ########################################################
            ...
            #########################################################
            ###      plot  mean and stdv of departures            ###
            #########################################################
...
#               print('   .... no data for: ', image_name)
            ...
            ########################################################
            ###       plot zonal departures mean and stdv        ###
            ########################################################
...
def plot_global_ob_sens(data, data_range, var_indx, ttl, image_name, include):
    ...
    for l, k in enumerate(sorted_raob_keys):
        if include == 'normal':
            if 'other' in k or 'profiler' in k:
                ...
                continue
        xses = data['ob_sens'][:,l,var_indx]
        yses = np.ma.array( yticks, mask=xses.mask )
        if not np.sum(data['sens_cnt'][:,l,var_indx]):
                continue
        line_color += 1
        lines[l] = plt.plot( -1.0*xses, yses, '-', linewidth=3.5, 
                             color=colors[line_color%len(colors)], label=k )
    ...
    plt.xlabel('24-hr error norm reduction',fontsize=font_base,fontweight='demibold')
    plt.ylabel('Pressure (hPa)',fontsize=font_base,fontweight='demibold')
...
def initialize_raob_dict( ly, lx, raob_types, var_types, REAL_TYPE):
    ...
    d['ob_sens'] = np.zeros((ly['num'], raob_types, var_types),dtype=REAL_TYPE)
    d['sens_cnt'] = np.zeros((ly['num'], raob_types, var_types),dtype=REAL_TYPE)
    ...
    d['zonal_ob_sens'] = np.zeros((ly['num'], lx['num'], raob_types, var_types),dtype=REAL_TYPE)
    d['zonal_sens_cnt'] = np.zeros((ly['num'], lx['num'], raob_types, var_types),dtype=REAL_TYPE)
